...
;(async () => {
  ...
  check('structured data carries bands and norms (compact text, not JSON)', /^\d+\. .+ \d\.\d\d — (Profound Strength|Promising Profound Strength|Above Average|Below Average|Potential Fatal Flaw) \(75th \d\.\d\d, 90th \d\.\d\d/m.test(full) && !/"norm_90th":/.test(full))
  check('strength builder index in the prefix and candidate entries in the snapshot', /STRENGTH BUILDERS \(Zenger Folkman/.test(full) && /STRENGTH BUILDERS FOR THE COMPETENCIES/.test(full))
