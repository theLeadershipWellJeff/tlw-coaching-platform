...
check('development target: sweet spot, novice, balance-the-tent in the guide and the prefix text', rg.development_target.sweet_spot.length === 2 && /Leadership Sweet Spot/.test(rgt) && /Novice/.test(rgt) && /balance your tent/.test(rgt))
...
check('reading guide ≤ 2,600 tokens', tokens(rgt) <= 2600, `${tokens(rgt)}`)
...
for (const f of fixtures) {
  ...
  check(`${name}: candidates labelled Sweet Spot / Novice route`, data.development_candidates.filter((c) => c.circles_met === 3).every((c) => text.includes(`${c.competency} ${c.total.toFixed(2)} — `) && text.includes(c.at_or_above_75th ? '[Sweet Spot route]' : '[Novice route]')))
  const profoundN = data.competency_rankings.filter((r) => r.band === 'Profound Strength').length
  check(`${name}: TENT BALANCE line (${profoundN} Profound)`, text.includes(`TENT BALANCE: ${profoundN} Profound Strength`) && (profoundN >= 5 ? /balances the tent/.test(text) : /does not apply yet/.test(text)))
  ...
  const cands = prompt.candidateCompetencies(data, prompt.builderEntryLimit(data))
  const builders = sb.renderStrengthBuilders(cands)
  const snapshotTokens = tokens(text) + tokens(builders)
  check(`${name}: candidates (${cands.join(', ')}) → entries in the guide; two on a long rendering`, cands.length >= 1 && cands.length <= 3 && cands.every((c) => sb.strengthBuildersFor(c)) && (text.length > prompt.LONG_RENDER_CHARS ? cands.length <= 2 : true))
