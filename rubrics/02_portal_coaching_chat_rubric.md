...
**Current version: v1.3** (brief body = `portal_chat` v1, migration 065; the code floor gained the documents-path and 360-status lines on 2026-09-08, the status line names the mismatched names and the Retry path, and since 2026-09-19 the prompt is budgeted and cached in prefix / snapshot / tail blocks) · September 2026 · Owner: Dr. Jeff Holmes
...
**Where it runs.** `lib/portal/prompt.ts#composeChatSystemParts`, assembled per message by `lib/portal/chat.ts#buildChatContextParts` and fitted to the context budget by `lib/portal/context.ts#buildChatRequest` (`lib/ai/context-budget.ts`). Layer order, in three cache-controlled blocks: **prefix** (cached 1 h, identical for every client of the org) = preamble (client-agnostic) → voice standards → **this rubric's brief (`portal_chat`, when active)** → assessment grounding + 360 brief (only with a report) → company context; **snapshot** (cached 5 min, this client) = who they are + the human route (+ a 360 status line) → structured 360 data → verbatims → goals → client documents → the client's own notes → coach-sent session notes; **tail** (never cached, per question) = a summary of conversation turns older than the last six → excerpts (the newest two sessions' openings + passages ranked against the question — never a full transcript). Replies stream; the model is routed by `lib/ai/models.ts` (purpose `portal_chat`: quality `frontier` → `claude-opus-5` at effort `medium`; past a client's soft cap the reply runs on `portal_degraded` → `claude-sonnet-5`; pin with `AI_MODEL_PORTAL_CHAT`); `max_tokens` 4,000 including thinking; input capped at 40k tokens (drop order excerpts → history → snapshot, never the prefix); every call metered on `ai_usage` and capped per client / org (migrations 069–071).
...
- **v1.3** (2026-09-19, code floor only, no brief change) — context budgeter: the prompt is sent as prefix / snapshot / tail blocks (above); the client's name and the human-route line move from the preamble to the snapshot ("WHO YOU ARE TALKING WITH") so the prefix caches across clients; only the last six turns ride verbatim (older turns summarised); excerpts replace full transcripts; `max_tokens` 4,000 including thinking.
