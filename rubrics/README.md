...
| 05 | [ZF 360 Strength Builders — reference](05_zf360_strength_builders_reference.md) | Vendor reference, not a rubric: Zenger Folkman's Strength Builder Development Guide (companion behaviors + development ideas per competency) as the assistant carries it | Data: `lib/documents/assessment-360/strength-builders.json` → index in the cached prefix + full entries for the report's candidates in the snapshot (`strength-builders.ts`, `lib/portal/prompt.ts`) | Edit the JSON, then `node scripts/rubrics/render-strength-builders.js` (`--check` to verify). Code deploy. |
