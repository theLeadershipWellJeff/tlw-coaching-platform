...
| 05 | [ZF 360 vendor reference — reading guide + Strength Builders](05_zf360_strength_builders_reference.md) | Vendor reference, not a rubric: Zenger Folkman's own reading guide (seven insights + the section-by-section questions) and its Strength Builder Development Guide (companion behaviors + development ideas per competency) as the assistant carries them | Data: `lib/documents/assessment-360/report-guide.json` (prefix text) + `strength-builders.json` → index in the cached prefix, full entries for the report's candidates in the snapshot (`report-guide.ts`, `strength-builders.ts`, `lib/portal/prompt.ts`) | Edit the JSON, then `node scripts/rubrics/render-strength-builders.js` (`--check` to verify). Code deploy. |
