...
${p.coachingBrief.body.trim()}`)
  ...
  if (p.assessment) {
    ...
    snapshot.push(
      `${clientName.toUpperCase()}'S MOST RECENT ASSESSMENT — STRUCTURED DATA (${p.assessment.data.instrument}, ${p.assessment.data.report_date}${count > 1 ? `; ${count} assessments on file, the most recent is given in full and its "comparison" block covers the change from the prior one` : ''}${p.assessment.data.reassessment ? '; this is a follow-up report and its "reassessment" block is the report\'s own comparison with the previous administration' : ''}). The sole source of truth for every number:\n${structured}`
    )
