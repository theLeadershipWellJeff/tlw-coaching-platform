...
export interface SuggestInput {
  ...
  /** Ledger attribution (purpose `scoring_suggest`, coach principal). */
  meta: { orgId: string | null; coachId: string | null; clientId?: string | null }
}
...
Give one specific move to raise competency ${c.id} (${c.name}) in the next session, grounded in what actually happened above.`
  ...
  const message = await aiCreate(
    { purpose: 'scoring_suggest', principal: 'coach', orgId: input.meta.orgId, coachId: input.meta.coachId, clientId: input.meta.clientId ?? null },
    {
      max_tokens: 400,
      system: SYSTEM,
