/**
 * Prep-sheet content generation — the ONE generator, shared by the interactive
 * flow (app/api/generate) and the background pipeline (lib/prep-sheets/generate).
 *
 * This is the extraction of what used to live inline in app/api/generate/route.ts:
 * same prompt, same source-precedence rules, same locked-goals overlay. Nothing
 * about the prep sheet's content, layout, or prompt changes here — it is lifted
 * verbatim so the pipeline reuses it rather than forking a second generator.
 *
 * Model resolution mirrors lib/scoring/engine.ts / lib/nudges/llm.ts: a stale
 * GENERATE_MODEL env pointing at a retired id falls back to the safe default,
 * and the resolved id is returned so the caller can record it for audit.
 */
...
/** Resolve the prep-generation model, guarding a retired GENERATE_MODEL env. */
export function resolvePrepModel(): string {
  const configured = process.env.GENERATE_MODEL?.trim()
  if (configured && RETIRED_MODELS.has(configured)) {
    console.warn(`GENERATE_MODEL "${configured}" is retired; falling back to ${SAFE_DEFAULT_MODEL}.`)
 ...
 *
 * NB: selects `coaching_goals` ONLY — never `clients.key_info` (the key-info wall).
 */
...
export type PrepNoteInput = { date?: string | number; content: string }

export type GeneratePrepInput = {
  clientName: string
  clientId?: string
  notes: PrepNoteInput[]
...
export type GeneratePrepResult = { content: PrepContent; model: string }

/**
 * Build the prep-sheet content from session context. Throws on a missing API key,
 * a Claude error, or an unparseable response — the caller decides how to surface
 * it (the route returns 500; the pipeline writes status='failed').
 */
export async function generatePrepContent(input: GeneratePrepInput): Promise<GeneratePrepResult> {
  const { clientName, clientId, notes, actions, zoomSummaries } = input
  ...
  const notesText = notes
    .map((n) => `[${n.date ? new Date(n.date).toLocaleDateString() : 'undated'}]\n${n.content}`)
    .join('\n\n---\n\n')

  const actionsText = actions?.length
    ? actions.map((a) => `• ${a.description}${a.dueDate ? ` (due ${a.dueDate})` : ''}`).join('\n')
    : 'None recorded'

  let zoomSection = ''
  if (zoomSummaries?.length) {
    const zoomText = zoomSummaries
      .map((s: any) => {
        const date = new Date(s.meeting_start_time).toLocaleDateString()
        const sections = (s.summary_details || [])
          .map((d: any) => `  ${d.label}: ${d.summary}`)
          .join('\n')
        const nextSteps = (s.next_steps || []).map((n: string) => `  • ${n}`).join('\n')
...
${jsonShape}`
  ...
  const raw = message.content.find((b) => b.type === 'text' && 'text' in b)
  const rawText = raw && 'text' in raw ? raw.text : ''
  const clean = rawText.replace(/```json\n?|```/g, '').trim()
  const jsonMatch = clean.match(/\{[\s\S]*\}/)

  const content = JSON.parse(jsonMatch ? jsonMatch[0] : clean) as PrepContent
