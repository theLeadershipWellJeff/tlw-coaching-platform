...
export async function sendScorecardEmail(coach: Coach, report: SessionReportJson): Promise<string | null> {
  const from = process.env.JEFF_FROM_EMAIL
  if (!from) throw new Error('JEFF_FROM_EMAIL is not configured.')
  if (!coach.email) throw new Error('Coach has no email address.')
  ...
  const raw = makeRawEmail(from, coach.email, subject, html)
