...
**`069_publish_goal_setting_brief_v2.sql` — APPLIED (production, confirmed
2026-09-10; data only, no schema; generated by `publish-brief.js weekly_plan
--sql`; idempotent).** Publishes
...
called"). The code floor (`GOAL_SETTING_FLOOR`) carries the same rules, so the
two stay in step. Reversible via `069_publish_goal_setting_brief_v2_down.sql`.
