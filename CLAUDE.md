...
- **`pipeline.ts`** — the ONE persistence path (coach upload, client
  ...
  (2026-09-08) — a PDF uploaded as kind `general` is fingerprinted
  (`index.ts#detectAssessment360`) and, when it is a supported 360 layout,
  promoted to `assessment_360` (caps, name gate, comparison, the assessments
  flag all apply; `UploadResult.promotedTo360` + a message say so; the three
  upload routes and the portal card key on `result.document.kind`, never the
  form's kind). Found when Jeff filed his own report as an "Other document"
  (the portal picker's default): it reached the chat as clipped plain text.
  A personnel review is never promoted (private by choice). PDF header + 4 MB
  check, per-client caps (5 assessments /
...
- **Verification.** `scripts/spikes/verify-portal-phase3.js` (no key needed):
  ...
  **not run in CI; run before the dry run.** `verify-portal-360-golden.js`
  (needs `ANTHROPIC_API_KEY` + the private PDF) runs the **golden set** in
...
- **Plan your week.** A second chat mode: `portal_conversations.mode`
  ...
  **`weekly_plan` brief** (seeded from Jeff's goal-setting master prompt;
  ...
  prescribe; rubrics/03 v1.1; v2.0 on 2026-09-10 added `GOAL_SETTING_FLOOR` —
  ask about a preferred framework first, SMART held lightly, one measure per
  goal is enough, three big engagement goals + the Top 5 — in BOTH chat modes,
  and the goals header no longer says Objectives/Key Results) → client documents →
...
**`049_coaching_hours_entries.sql` — APPLIED (staging + production, 2026-08-14).**
Coaching hours import (canonical hours record).
...
pro-bono split), RLS enabled. Additive; the log GET and the PDF export read the
table defensively (they degrade to notes-only if it's missing), and the import
POST and imported-row edit/delete routes use it. `lib/coaching-hours.ts` is the
...
**`069_publish_goal_setting_brief_v2.sql` — PENDING (data only, no schema;
generated by `publish-brief.js weekly_plan --sql`; idempotent).** Publishes
the `weekly_plan` brief **v2** from rubrics/03 v2.0 (SMART held lightly, the
framework question at the start of goal setting, three big engagement goals +
the Top 5, one measure per goal, "remember my goals whatever they are
called"). Until it is applied the portal still runs the 061-seeded v1 brief
over the new code floor (`GOAL_SETTING_FLOOR`), which already carries the
same rules. Reversible via `069_publish_goal_setting_brief_v2_down.sql`.
...
report; the active brief for every 360 conversation from the next message).
Generated by `publish-brief.js assessment_360 --sql`; idempotent; safe before or
after 065. Reversible via the down-script.

**`065_publish_rubric_briefs.sql` — APPLIED (production, 2026-09-09; data only,
no schema; `portal_chat` v1 from rubrics/02).** ⚠️ Trimmed that day to the
`portal_chat` block only: it originally also published `assessment_360` v2, but
066 had already landed the newer v2.1 while 065 sat unapplied, and running the
stale block afterwards would have inserted v2's body as a fresh version and
ACTIVATED it — rolling 066 back. Rule: **a brief-publish migration that is
overtaken by a later one for the same slug must be trimmed, never run as-is.**
Generated by `publish-brief.js portal_chat --sql`; idempotent; reversible via
`065_publish_rubric_briefs_down.sql` (portal_chat only now).
